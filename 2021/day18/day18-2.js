...
function copyNum(num) {
  return [
    Array.isArray(num[0]) ? copyNum(num[0]) : num[0],
    Array.isArray(num[1]) ? copyNum(num[1]) : num[1],
  ];
}
...
function run(input) {
  ...
  for (let i = 0; i < numbers.length; i++) {
    for (let j = i + 1; j < numbers.length; j++) {
      const sum = add(copyNum(numbers[i]), copyNum(numbers[j]));
  ...
  console.log('number:', JSON.stringify(largestMagNum));
  console.log('magnitude:', largestMagnitude);
}
